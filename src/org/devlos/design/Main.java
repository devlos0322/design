...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        */
        /* 데코레이터 패턴
            1. 기존 뼈대(클래스)는 유지하되, 이후 필요한 형태로 꾸밀 때 사용한다.
            2. 확장이 필요한 경우 상속의 대안으로도 활용된다.
            3. SOLID 중에서 개방 폐쇠 원칙(OCP) 과 의존 역전 원칙(DIP)를 따른다.
            ex) 자동차를 가지고 예제를 진행. 등급에 따라 가격이 바뀜
         */
        /*
        ...
        */
        /* Observer pattern
            1. 관찰자 패턴은 변화가 일어 났을 때, 미리 등록된 다른 클래스에 통보해주는 패턴을 구현한 것이다.
            2. 많이 보이는 곳은 이벤트 리스너가 이 패턴의 표준적인 예.
         */
        /*
         ...
         */
        /* Facade pattern
            1. Facade는 건물의 앞쪽 정면 이라는 뜻을 가진다.
            2. 여러 개의 객체와 실제 사용하는 서브 객체의 사이에 복잡한 의존관계가 있을 때, 중간에 facade라는 객체를 두고, 여기서 제공하는 interface 만을 활용하여 기능을 사용하는 방식이다.
            3. Facade는 자신이 가지고 있는 각 클래스의 기능을 명확힐 알아야 한다.
            ex) Client가 FTP 기능을 사용할 경우, FTP, WRITER, READER 클래스의 open close를 모두 명시해서 사용해야하는데, 이것을 간편하게 사용할 수 있도록 한다.
         */
        /*
        ...
        */
        /* 전략패턴
            1. 객체지향의 꽃.
            2. 유사한 행위들을 캡슐화 하여, 객체의 행위를 바꾸고 싶은 경우 직접 변경하는 것이 아닌 전략만 변경하여, 유연하게 확장하는 패턴 SOLID 중에서 개방폐쇠 원칙 (OCP)과 의존 역전 원칙(DIP)를 따른다.
            3. 전략 메서드를 가진 전략 객체. 인코딩 (Normal Strategy, Base64 Strategy)
         */
        //전략 객체를 사용하는 컨텍스트(Encoder): 전략 객체를 생성해 컨텍스트에 주입하는 클라이언트
